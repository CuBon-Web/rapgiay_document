    ...
    saveProducts() {
      ...
      this.saveProduct(payload)
        .then(() => {
          ...
          this.$success("Cập nhật tài liệu thành công");
        })
        .catch(() => {
          this.loadings(false);
        });
    ...
    editById() {
      ...
      this.editId({ id: this.$route.params.id })
        .then((productResponse) => {
          ...
          this.objData.images = this.parseJsonField(data.images, []);
          this.objData.preserve = this.parseJsonField(data.preserve, []);
          this.objData.content = this.parseJsonField(data.content, this.objData.content);
          this.objData.description = this.parseJsonField(data.description, this.objData.description);
          this.objData.tags = this.parseJsonField(data.tags, []);
          this.objData.origin = data.origin || "";
          this.withSyncPause(() => {
            this.seoData.seoTitle = data.seo_title || this.objData.name || "";
            this.seoData.metaDescription =
              data.meta_description || (this.objData.description[0] && this.objData.description[0].content) || "";
            this.seoData.focusKeyword = data.focus_keyword || "";
            this.seoData.slug = data.slug || this.slugifySeo(this.objData.name || "");
          });
          this.findTypeCate(this.objData.category).then((response) => {
            this.type_cate = response.data;
          });
        ...
        .catch(() => {
          this.$error("Không thể tải dữ liệu tài liệu");
        })
        .finally(() => {
          this.loadings(false);
        });
