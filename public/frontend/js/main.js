...
--------------------------------------------- */
  ...
  var swiper = new Swiper(".newest-slider:not(.category-newest-slider)", {
    slidesPerView: 1,
    spaceBetween: 30,
    loop: true,
    navigation: {
...
$(".swiper-slide .nav-item .nav-link ").on("click",function(){  
  ...
  if (sidebarButton) {
    sidebarButton.addEventListener("click", function () {
      var sidebarMenu = document.querySelector(".main-menu, .sidebar-menu");
      if (sidebarMenu) {
  ...
  // Product detail thumbnail up/down controls
  (function initDetailThumbScroller() {
    var $wrap = $(".detail-thumb-nav-wrap");
    if (!$wrap.length) return;

    $wrap.each(function () {
      var $currentWrap = $(this);
      var $viewport = $currentWrap.find(".detail-thumb-nav-viewport");
      var $nav = $currentWrap.find(".detail-thumb-nav");
      var $prev = $currentWrap.find(".detail-thumb-prev");
      var $next = $currentWrap.find(".detail-thumb-next");

      function updateArrowState() {
        var isMobile = window.matchMedia("(max-width: 576px)").matches;
        var maxScroll = isMobile
          ? $nav.outerWidth() - $viewport.innerWidth()
          : $nav.outerHeight() - $viewport.innerHeight();
        ...
        $next.show();
        if (isMobile) {
          $prev.prop("disabled", $viewport.scrollLeft() <= 0);
          $next.prop("disabled", $viewport.scrollLeft() >= maxScroll - 1);
        } else {
          $prev.prop("disabled", $viewport.scrollTop() <= 0);
          $next.prop("disabled", $viewport.scrollTop() >= maxScroll - 1);
        }
      }

      $prev.on("click", function () {
        var isMobile = window.matchMedia("(max-width: 576px)").matches;
        var step = isMobile ? $currentWrap.find(".nav-link").first().outerWidth(true) || 80 : 105;
        if (isMobile) {
          $viewport.stop().animate({ scrollLeft: $viewport.scrollLeft() - step }, 180, updateArrowState);
        } else {
          $viewport.stop().animate({ scrollTop: $viewport.scrollTop() - step }, 180, updateArrowState);
        }
      });
      ...
      $(window).on("resize", updateArrowState);
      updateArrowState();
    });
  })();
